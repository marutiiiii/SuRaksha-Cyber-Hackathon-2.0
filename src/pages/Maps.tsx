...
function Card({ map }: { map: MAP }) {
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className={`bg-card border border-border rounded-md p-3 shadow-sm cursor-grab active:cursor-grabbing ${isDragging ? "opacity-50" : ""}`}
    >
...
function Column({ status, cards, onOpen }: { status: MapStatus; cards: MAP[]; onOpen: (m: MAP) => void }) {
  ...
  return (
    <div ref={setNodeRef} className={`bg-muted/40 border border-border rounded-md flex flex-col min-h-[400px] ${isOver ? "ring-2 ring-primary" : ""}`}>
