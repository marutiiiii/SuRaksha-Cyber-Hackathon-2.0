...
def verify_evidence_background(evidence_id: UUID, db: Session = None):
    """
    ...
    # Establish a fresh isolated session for this background thread
    ...
    try:
        try:
            ...
            # Determine status transition based on matching score:
            # - 100% completed (score == 1.0 or status == COMPLETED) -> Completed
            # - 0% completed (score == 0.0) -> Pending (reverts)
            # - Partial completed (0 < score < 1.0) -> In Progress
            if status_result["status"] == "COMPLETED" or match_result["score"] >= 1.0:
                ev.validation_status = "Passed"
                ev.rejection_reason = None
                map_task.status = "Completed"
                print(f"[{evidence_id}] Compliance verified 100%. Transitioning MAP status to Completed.")
            elif match_result["score"] <= 0.0 or len(match_result.get("matched", [])) == 0:
                ev.validation_status = "Failed"
                ev.rejection_reason = status_result["reason"]
                map_task.status = "Pending"
                print(f"[{evidence_id}] Compliance failed 0%. Reverting MAP status to Pending.")
            else:
                ev.validation_status = "Failed"
                ev.rejection_reason = status_result["reason"]
                map_task.status = "In Progress"
                print(f"[{evidence_id}] Compliance partial ({int(match_result['score']*100)}%). Transitioning MAP status to In Progress.")
