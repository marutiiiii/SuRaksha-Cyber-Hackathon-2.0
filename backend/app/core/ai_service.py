...
class LlamaAIService:
    ...
    def copilot_chat(cls, message: str, context: str, maps_context: str) -> str:
        ...
        system_prompt = (
            ...
            "Respond in a natural, conversational, and user-friendly manner that directly matches the user's intent:\n"
            "- If they ask for a summary, provide a concise, structured executive summary.\n"
            "- If they ask for compliance requirements, checklist, or steps, provide a clear, action-oriented list of compliance steps.\n"
            "- If they ask for an explanation or detailed guidance, write a thorough, detailed explanation of the rules and their impacts.\n"
            "- If they ask for a comparison, contrast the different rules, versions, or regulations.\n"
            "Do not enforce a rigid header structure. Let the formatting flow naturally based on the query. "
            "Cite referenced context sources using bracket numbers (e.g., [1], [2]) at the end of relevant sentences. "
            "Avoid dumping raw clauses directly."
        )
        prompt = (
